...
class SymbolicItem(object):
    ...
    def copy(self):
        return SymbolicItem(self.addr, self.obj, self.t, self.obj)
...
class SymbolicMemory(simuvex.plugins.plugin.SimStatePlugin):
            ...
            if type(size) in (int, long):
                ...
                for k in range(size):
                    ...
                    if condition is not None:
                        ...
                        if constant_addr:
                            ...
                            if len(P) == 1:
                                ...
                                self._concrete_memory[min_addr + k] = SymbolicItem(min_addr + k, obj, self.timestamp, None)
                                replaced = True
                    ...
                    if not replaced:
                        ...
                        if constant_addr and condition is None:
                            if self.verbose: self.log("\tAdding concrete address...")
                            self._concrete_memory[min_addr + k] = SymbolicItem(min_addr + k, obj, self.timestamp, None)
                        else:
                            if self.verbose: self.log("\tAdding node...")
                            self._symbolic_memory.add(min_addr + k, max_addr + k + 1, SymbolicItem(addr + k, obj, self.timestamp, condition))
            ...
            for page_index in page_indexes:
                ...
                for offset in offsets:
                    ...
                    if not same_value:
                        ...
                        self._concrete_memory[page_index * 0x1000 + offset] = merged_value
    ...
    def _copy_symbolic_items_and_apply_guard(self, L, guard):
        ...
        for l in L:
            l = l.copy()
            l.guard = claripy.And(l.guard, guard) if l.guard is None else guard
            LL.append(l)
        return LL
